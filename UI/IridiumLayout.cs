...
public static class IridiumLayout
{
    ...
    /// <summary>UMM 的 UI 缩放（×1048576）。UMM 不可用时退回 1.0。</summary>
    private static int GetUIScaleTimes1M()
    {
        try { return (int)UnityModManagerNet.UnityModManager.UI.Scale(1048576); }
    ...
    public static IStructFormat<float> FloatFormat(
        int? precision = null,
        float min = float.NegativeInfinity,
        float max = float.PositiveInfinity
    )
    {
        return new FloatFormatImpl(precision, min, max);
    }
    ...
    private sealed class FloatFormatImpl(
        int? precision,
        float lower,
        float upper
    ) : IStructFormat<float>
    {
        ...
        {
            return precision is null ? value.ToString("R") : value.ToString($"F{precision}");
        }

        public float? Parse(string text)
        {
            if (string.IsNullOrEmpty(text)) return 0f;
            if (!float.TryParse(text, out var result)) return null;
    ...
    internal static class Engine
    {
        ...
        private static GUIStyle OffsetStyle(GUIStyle source)
        {
            ...
            var sizesScope = SizesScopes[SizesScopes.Count - 1];
            var alignmentScope = AlignmentScopes[AlignmentScopes.Count - 1];
        ...
        {
            if (ContainerStack[ContainerStack.Count - 1] != ContainerDirection.Horizontal)
        ...
        internal static void UpdateMaxSize()
        {
            ...
            var isHorizontal = ContainerStack[ContainerStack.Count - 1] == ContainerDirection.Horizontal;
        ...
        )
        {
            ...
            Frames.Add(new Frame
            {
                ...
                ApplyPreMarginHorizontal = style == ContainerStyle.None && Frames[Frames.Count - 1].ApplyPreMarginHorizontal,
                ApplyPreMarginVertical = style == ContainerStyle.None && Frames[Frames.Count - 1].ApplyPreMarginVertical
            });
            PushSizes(sizes);
        }
        ...
        internal static void End()
        {
            ...
            if (direction == ContainerDirection.Horizontal)
            {
                ...
                SizesScopes.RemoveAt(SizesScopes.Count - 1);
                Frames[Frames.Count - 1].ApplyPreMarginHorizontal = true;
            }
            else
            {
                ...
                Frames.RemoveAt(Frames.Count - 1);
                SizesScopes.RemoveAt(SizesScopes.Count - 1);
                Frames[Frames.Count - 1].ApplyPreMarginVertical = true;
            }
        }
    ...
    private class ResolutionResources
    {
        ...
        private static void LoadColors()
        {
            ...
            try
            {
                ...
                Main.Mod?.Logger.Log($"Loaded {_loadedColors.Count} colors from Colors.iml");
            }
            catch (Exception ex)
            {
                Main.Mod?.Logger.Log($"Failed to load Colors.iml: {ex.Message}");
            }
        }
        ...
        private Texture2D RenderImage(int width, int height, Action<Canvas> renderer)
        {
        ...
        private Texture2D RenderRoundedRect(int width, int height, double radius, Color color)
        {
            return RenderImage(width, height,
                canvas => canvas.FillRoundedRect(0, 0, width, height, (float)radius, color));
        }
        ...
        )
        {
            ...
            {
                canvas.FillRoundedRect(0, 0, width, height, (float)radius, borderColor);
                canvas.FillRoundedRect(
                    (float)border,
                    (float)border,
                    width - (float)border * 2f,
                    height - (float)border * 2f,
                    (float)(radius - border),
                    color
                );
            });
        ...
        private Texture2D RenderSquareGlyph(
            ...
            Color borderColor,
            Color strokeColor,
            Action<Canvas, int, Color> stroke
        )
        {
            ...
            {
                canvas.FillRoundedRect(0, 0, size, size, (float)radius, borderColor);
                canvas.FillRoundedRect(
                    (float)border,
                    (float)border,
                    size - (float)border * 2f,
                    size - (float)border * 2f,
                    (float)(radius - border),
                    color
                );
        ...
        )
        {
            ...
            return RenderImage(width, height, canvas =>
            {
                canvas.FillRoundedRect(0, 0, width, height, radius, color);
        ...
        )
        {
            ...
            return RenderImage(size, size, canvas =>
            {
                canvas.FillCircle(size / 2F, size / 2F, size / 2F, borderColor);
                canvas.FillCircle(size / 2F, size / 2F, size / 2F - border, color);
                stroke(canvas, size, strokeColor);
            });
        }

        private static void DrawCheckmark(Canvas canvas, int size, Color strokeColor)
        {
            ...
            ], size * 2 / 20F, strokeColor);
        }

        private static void DrawArrow(Canvas canvas, int size, Color strokeColor, bool flip, bool rotate)
        {
            canvas.StrokePolyline([
                Transform(size * 13 / 32F, size * 8 / 32F),
                Transform(size * 21 / 32F, size * 16 / 32F),
                Transform(size * 13 / 32F, size * 24 / 32F)
            ], size * 2 / 20F, strokeColor);
            ...
            Vector2 Transform(float x, float y)
            {
                ...
                var py = y;
                if (flip) px = size - px;
                if (rotate) (px, py) = (size - py, px);
                return new Vector2(px, py);
            }
        }

        private static void DrawRightArrow(Canvas canvas, int size, Color strokeColor)
        {
            DrawArrow(canvas, size, strokeColor, false, false);
        }

        private static void DrawDownArrow(Canvas canvas, int size, Color strokeColor)
        {
            DrawArrow(canvas, size, strokeColor, false, true);
        }

        private static void DrawLeftArrow(Canvas canvas, int size, Color strokeColor)
        {
            DrawArrow(canvas, size, strokeColor, true, false);
        }

        private static void DrawUpArrow(Canvas canvas, int size, Color strokeColor)
        {
            DrawArrow(canvas, size, strokeColor, true, true);
        }

        private static void FillDot(Canvas canvas, float x, float y, float diameter, Color strokeColor)
        {
            canvas.FillCircle(x + diameter / 2F, y + diameter / 2F, diameter / 2F, strokeColor);
        }

        private static void DrawRingWithStem(Canvas canvas, int size, Color strokeColor, float stemTop, float stemBottom)
        {
            var thickness = size * 1.5F / 20F;
            canvas.StrokeArc(
                size * 10 / 20F,
                size * 10 / 20F,
                size * 6 / 20F,
        ...
        private static void DrawInformation(Canvas canvas, int size, Color strokeColor)
        {
            DrawRingWithStem(canvas, size, strokeColor, 9.5F, 13F);
            FillDot(canvas, size * 9.25F / 20F, size * 6.25F / 20F, size * 1.5F / 20F, strokeColor);
        }

        private static void DrawSuccess(Canvas canvas, int size, Color strokeColor)
        {
            var thickness = size * 1.5F / 20F;
            canvas.StrokeArc(
                size * 10 / 20F,
            ...
            ], thickness, strokeColor);
        }

        private static void DrawWarning(Canvas canvas, int size, Color strokeColor)
        {
            var thickness = size * 1.5F / 20F;
            canvas.StrokePolyline([
            ...
            ], thickness, strokeColor, closed: true);
            canvas.StrokeLine(
                size * 10 / 20F,
                size * 7 / 20F,
                size * 10 / 20F,
            ...
            FillDot(canvas, size * 9.25F / 20F, size * 12.25F / 20F, size * 1.5F / 20F, strokeColor);
        }

        private static void DrawError(Canvas canvas, int size, Color strokeColor)
        {
        ...
        private static void DrawStop(Canvas canvas, int size, Color strokeColor)
        {
